...
SELECT customer_last_name, customer_first_name, spend
FROM customer c
INNER JOIN  
(SELECT customer_id,
...
SELECT customer_last_name, customer_first_name, market_date, spend 
FROM customer c
JOIN (
SELECT customer_id, market_date, SUM(quantity * cost_to_customer_per_qty) AS spend 
FROM customer_purchases
WHERE strftime('%Y-%m', market_date) LIKE '%2022-04%'
